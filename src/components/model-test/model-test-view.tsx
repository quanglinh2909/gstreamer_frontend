...
export function ModelTestView() {
    ...
    // Trên mobile KHÔNG tách hai cột mà xếp chồng, cho cả trang cuộn: cột điều
    // khiển rộng 320px cạnh màn 390px thì khung ảnh chỉ còn 70px — mà ảnh mới
    // là thứ cần nhìn.
    return (
        <div className="flex h-full min-h-0 flex-col overflow-y-auto md:flex-row md:overflow-hidden">
            {/* CỘT ĐIỀU KHIỂN */}
            <aside className="flex shrink-0 flex-col gap-4 border-b border-slate-200 p-4 md:w-80 md:overflow-y-auto md:border-b-0 md:border-r">
                <div>
            ...
            {/* KHUNG ẢNH + KẾT QUẢ */}
            <main className="flex min-w-0 flex-1 flex-col bg-slate-100 p-4 md:overflow-auto md:p-6">
                {imageUrl ? (
