...
import { getVisibleParkingLotEventPages } from "@/lib/parking-lot-event-view-model";
import { cn } from "./parking-lot-event-utils";

export function ParkingLotEventPagination({
    currentPage,
    totalPages,
    onPageChange,
}: {
    ...
    const visiblePages = getVisibleParkingLotEventPages(currentPage, totalPages);
    ...
    return (
        <nav aria-label="Phân trang sự kiện bãi xe" className="flex flex-wrap items-center justify-center gap-2">
            <button
                type="button"
